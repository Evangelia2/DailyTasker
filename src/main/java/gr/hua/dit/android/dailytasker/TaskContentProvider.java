...
public class TaskContentProvider extends ContentProvider {
    ...
    public Cursor query(@NonNull Uri uri, @Nullable String[] projection, @Nullable String selection,
        ...
        if (cursor != null) {
            Log.d("TaskContentProvider", "Query returned " + cursor.getCount() + " rows.");
            cursor.setNotificationUri(getContext().getContentResolver(), uri);
        } else {
            Log.d("TaskContentProvider", "Query returned null cursor.");
        }
    ...
    public Uri insert(@NonNull Uri uri, @Nullable ContentValues values) {
        ...
        Thread thread = new Thread(() -> {
            ...
            if (id != -1) {
                newUri[0] = ContentUris.withAppendedId(CONTENT_URI, id);
                getContext().getContentResolver().notifyChange(newUri[0], null);
            }
        });
        thread.start();

        try {
            thread.join();  // Wait for the operation to finish
